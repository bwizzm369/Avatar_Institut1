...
Administration : accès uniquement si session Supabase + `profiles.role = 'admin'` vérifié côté serveur (layout / actions). Pas de signup admin public. Élévation de rôle uniquement via SQL / service role (trigger anti auto-élévation). L’inscription publique ignore `role` / Student Pass / `legacy_match_status` envoyés par le navigateur.
...
- **Student Pass / Digital Membership** : `/dashboard/student-pass` affiche une carte membre (nom, Member ID public `AVT-M-` + 8 hex de `profiles.id`, ACTIVE/INACTIVE via `hasActiveStudentPass`). Si inactif : 3 formules Stripe Test — monthly 12 €, semiannual 72 € / 6 mois, annual 144 €/an (`STRIPE_STUDENT_PASS_*_PRICE_ID`). Le navigateur n’envoie que `plan`. Activation uniquement via webhook. Admin manuel/offline inchangé + source `stripe`. **Aucune migration**.
...
- **Reviews Lot A (local, non appliqué en distant)** : `public.reviews` étendu (`profile_id`, `rating` 1–5, `moderation_status` pending/approved/rejected, `reviewed_at` / `reviewed_by`). Insert étudiant = toujours pending + `is_published=false`, `profile_id = auth.uid()`. Une avis par profil (index unique partiel). Public = approved + published.
- **Reviews Lot B (local)** : formulaire étudiant sur `/reviews` (note 1–5, texte, confirmation) ; Header EN Reviews / AR الآراء ; admin Approve/Reject ; étoiles publiques pour les avis notés. Pas d’UI de redesign.
- **Reviews statut propre** : SELECT RLS `reviews_select_own` (`profile_id = auth.uid()`) ; plus de lecture service-role pour le statut étudiant. Migration Lot A locale toujours non appliquée en distant.
- **Préparation beta Vercel (non déployée)** : QR / PDF / auth emails lisent `NEXT_PUBLIC_APP_URL` uniquement (jamais d’hôte Vercel inventé). Tracing Template 2 + `maxDuration=60` sur les routes PDF. PDF officiel : Chrome/Edge local ; Vercel utilise `puppeteer-core` + `@sparticuz/chromium` (Template 2 inchangé).
...
7. Administration. ← **Lot 1 + Lot 2 + Lot 2B + Certificats 3A/3B/4A/4B + Student Pass Stripe Test livrés localement** (révocation certificats / Google Sheets / emails : lots suivants)
...
- Redirect URLs Auth : `{NEXT_PUBLIC_APP_URL}/auth/callback` (signup confirmation + password reset).
...
- Reviews Lot A : appliquer manuellement `20260829120000_reviews_student_moderation.sql` sur le projet Supabase distant quand autorisé.
- Reviews Lot B : valider le formulaire étudiant + Approve/Reject admin après application de la migration Lot A.
- Beta Vercel : poser `NEXT_PUBLIC_APP_URL` = origine HTTPS beta exacte (pas localhost). Ajouter `{BETA_URL}/auth/callback` dans Supabase Redirect URLs.
- Beta PDF : runtime Vercel via `@sparticuz/chromium` livré localement, **non encore déployé**. Confirmer le téléchargement PDF sur le projet beta après le premier deploy.
- Vérifier que `issue_certificate` existe déjà en distant avant d’appliquer `20260818160000_issue_certificate.sql`.
- Consultation/avis : `20260828120000_consultation_and_reviews.sql` puis `20260829120000_reviews_student_moderation.sql` avant TEST C.
